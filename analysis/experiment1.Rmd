...
blended <- read_csv(here("data/exp1/model_output/blended.csv")) %>%
  select(-n) %>%
  mutate(cdf = cdf / 10) %>%
  group_by(wordpair, step, cost_weight) %>%
  mutate(model_prob = softmax(5 * cdf)) %>%
  rename(correctedClue = Word)

blended %>%
  right_join(d.exp1) %>% 
  group_by(step, cost_weight) %>%
  summarize(loglik = sum(log(model_prob), na.rm = TRUE)) %>%
  ggplot(aes(x = step, y = loglik, color = cost_weight, group = cost_weight)) +
    geom_line() +
    #geom_point(data = . %>% group_by(step) %>% filter(loglik == max(loglik))) +
    scale_x_log10() +
    theme_few() +
    labs(x = "cost weight") +
````
...
```{r}
best_vals <- d_exp1 %>%
  left_join(blended, relationship = "many-to-many") %>%
  group_by(cost_weight, dist_weight, step, Level) %>%
  summarize(loglik = sum(log(prob), na.rm = TRUE)) %>%
  filter(!is.na(step)) %>%
  group_by(Level, cost_weight, dist_weight) %>%
  filter(loglik == max(loglik))

best_vals %>%
  ggplot(aes(x = as.numeric(cost_weight), y = loglik,
             color = dist_weight, group = dist_weight)) +
    geom_line() +
    facet_wrap(~ Level, scales = 'free') +
    theme_few() +
    labs(x = "search budget") +
>>>>>>> 74cec5631286437d3c7b71ec4110c525df42a859
