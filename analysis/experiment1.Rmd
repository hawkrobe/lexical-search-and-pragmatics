...
```{r}
forage_scores = read_csv(here("data/exp1/forage_clue_scores.csv")) %>%
  mutate(LL = log(score)) 

exp1Data = read_csv(here("data/exp1/exp1-cleaned.csv")) %>%
  left_join(forage_scores)

high_scoring_clues = exp1Data %>%
  group_by(wordpair,seqOrder ) %>%
  slice_max(LL) %>%
  select(-seqOrder) %>%
...
sumLL = exp1Data %>%
  ...
  arrange(Level,seqOrder, desc(sumLL))

sumLL %>%
  ggplot(aes(x = alpha, y = sumLL, color = seqOrder)) +
  geom_line(size=1)+
  facet_wrap(~Level)+
  theme_few()
