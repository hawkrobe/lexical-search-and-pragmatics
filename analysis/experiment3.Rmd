...
speaker_listener %>%
  ...
  theme_few() +
  scale_color_manual(values = c('#60BD68', '#BAB504', '#CC253D', "pink"))+
...
diag_plot = model_plus_ratings %>%
  ...
  mutate(`model\npredictions` = scales::rescale(prob),
         `human\nratings` = scales::rescale(response)) %>%
  pivot_longer(names_to = "metric", cols = c(`model\npredictions`, `human\nratings`)) %>%
  group_by(diagnosticity, metric) %>%
  tidyboot_mean(value, nboot = 1000, na.rm = T)  %>%
  mutate(metric = fct_relevel(metric, "human\nratings", "model\npredictions"))

diag_plot %>% 
ggplot(aes(x = metric, y = empirical_stat, group = diagnosticity, fill = diagnosticity)) +
  geom_bar(stat = 'identity', position = "dodge") +
    geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0, position =  position_dodge(0.9)) +
    labs(y = 'normalized ratings/predictions', x= "") +
    theme_few() +
    theme(aspect.ratio = 1, legend.position = c(0.8, 0.8)) +
    scale_fill_manual(values = c('darkslategray4',  '#CC253D'))

ggsave('plots/exp3_model_diag.pdf', units = 'in', height = 4, width = 4)
