...
```{r}
exp4.raw <- read_csv("../data/exp4/raw.csv") %>%
...
wordpair_model =lmer(data = num_candidates, utterance ~ wordpair + (1|clueGiverID))
car::Anova(wordpair_model)
nobs(wordpair_model)
...
suppression_data %>%
  ...
  group_by(level) %>% 
  tidyboot_mean(suppress, nboot = 1000, na.rm = T) %>% 
...
clueCounts %>%
  ...
  geom_col() +
  geom_text(aes(label = paste(correctedClue, round(prop,2), sep = ":")), 
            position = position_stack(vjust = 0.25), size = 7) +  
  theme_classic()+
  theme(legend.position = "none")+
  facet_wrap(~wordpair)+
  labs(title = "", y = "utterance proportion", x = "") +
...
ggsave('plots/exp4_snake_ash.pdf', units = 'in', height = 7, width =15)
```
...
clues <- read_csv(here('data/exp4/cleaned.csv')) %>%
  group_by(wordpair, seqOrder, correctedClue) %>%
  tally(name = 'clueCount') 

getWalks <- function(pair) {
  paste0('data/exp4/model_output/', pair, '-walks.json', collapse = '') %>%
    here() %>%
    fromJSON() %>% 
    as_tibble() %>%
...
ggsave(here(paste0('plots/cdf-duo.pdf')), units='in', height = 4, width = 8)
```
...
```{r}
targets = read_csv(here("../data/exp2/targets.csv"))
cleaned = read_csv(here("../data/exp2/cleaned.csv")) %>%
  left_join(targets)

cleaned_final = cleaned %>% filter(seqOrder == "clueFinal")

write.csv(cleaned_final, file = "../data/exp2/cleaned_clueFinal.csv", row.names = FALSE)

cdf = read_csv(here("../data/exp2/model_output/freqs_long.csv"))

head(cdf)

cleaned_complete = read_csv(here("../data/exp2/cleaned_complete.csv")) 
candidates = cleaned_complete %>% filter(seqOrder != "clueFinal")
write.csv(candidates, file = "../data/exp2/cleaned_candidates.csv", row.names = FALSE)
```
