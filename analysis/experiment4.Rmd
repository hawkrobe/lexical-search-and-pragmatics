...
```{r}
exp4_complete <- read_csv(here("data/exp4/cleaned.csv")) %>%
...
ggsave(here('analysis/plots/exp4_order.pdf'), units = 'in', width = 4, height = 2)
```
...
ggsave(here('analysis/plots/exp4_order_difficulty.pdf'), 
...
ggsave(here('analysis/plots/exp4_lion_tiger.pdf'), units = 'in', height = 5, width =6)
...
ggsave(here('analysis/plots/exp4_snake_ash.pdf'), units = 'in', height = 5, width =6)
```
...
d.spearmancor <- walks %>%
  left_join(clues %>% filter(seqOrder %in% c('clueOption2', 'clueOption1', 'clueOption3')), 
...
d.spearmancor.toplot %>%
  ungroup() %>%
#  mutate(seqOrder = fct_relevel(seqOrder, 'clueOrder1', 'clueOrder2', 'clueOrder3')) %>%
  filter(step %in% round(1.5**seq(1,30))) %>%
  ggplot(aes(x = step, y = r, color = level, linetype = seqOrder)) +
    geom_line() +
    theme_few() +
    scale_color_manual(values = c('#60BD68', '#D8CF23', '#CC253D')) +
    facet_grid(~ level)+
...
```{r}
walks %>%
  filter(wordpair %in% c('lion-tiger', 'trauma-weird')) %>%
  left_join(clues) %>%
...
ggsave(here(paste0('analysis/plots/cdf-duo.pdf')), 
...
ggsave(here(paste0('analysis/plots/cdfs-all.pdf')), 
units='in', height = 4, width = 8)
```
...
bind_rows(
  ...
  rename(wordpair = `wordpair...4`) %>%
  left_join(read_csv(here("data/exp4/model_output/freq_ranks.csv")) %>% mutate(src = 'empirical'), 
            by = c('clueGiverID', 'correctedClue', 'seqOrder', 'Level', 'src', 'wordpair')) %>% 
  left_join(read_csv(here("data/exp4/model_output/mixture_ranks.csv")) %>% 
...
d.scores <- bind_rows(
  ...
  filter(seqOrder %in% c('clueFinal')) %>%
