...
```{r}
vocab <- read_csv(here('data/exp1/model_input/vocab.csv')) 
...
ggsave(here('analysis/plots/exp1_order_difficulty.pdf'), 
...
d.spearmancor %>%
  ...
  ggplot(aes(x = step, y = r, color = level)) +
    geom_line(aes(group = wordpair), alpha = 0.1, data = d.spearmancor) +
    geom_line() +
    theme_few() +
    scale_color_manual(values = c('#60BD68', '#D8CF23', '#CC253D')) +
    facet_grid(~ level)+
...
ggsave(here('analysis/plots/exp1-spearman.pdf'), units = 'in', width = 8, height = 4)
```
...
bind_rows(
  ...
  ungroup() %>%
  mutate(level = fct_relevel(level, 'Easy', 'Medium', 'Hard')) %>%
  mutate(measure = fct_relevel(measure, 'freq', 'mid', 'single','union', 'intersection')) %>%
