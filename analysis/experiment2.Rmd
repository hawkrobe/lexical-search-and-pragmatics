...
```{r}
d.exp2 <- here("data/exp2/corrected.csv") |> 
  read_csv() |>
  distinct() |>
  select(wordpair, correctedClue, clueCount, Level)

search <- read_csv(here("data/exp2/model_input/scores.csv")) %>%
...
```{r}
pairs <- clues %>% pull(wordpair) %>% unique()
