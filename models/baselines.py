...
class Baseline:
    ...
    def save_midpoint_scores(self, data_path):
        ...
        for name, group in expdata.groupby('wordpair') :
            ...
            for clue in clue_list:
                index = closest_to_midpoint.index(clue) if clue in closest_to_midpoint else 10000
    ...
    def save_frequency_scores(self,data_path):
        ...
        for name, group in expdata.groupby('wordpair') :
            clue_list = group['correctedClue'].to_numpy()
            for clue in clue_list:
                index = freq.index(clue) if clue in freq else 10000
...
if __name__ == "__main__" :
    baseline = Baseline()
    baseline.save_midpoint_scores('../data/exp1/e1_data_long.csv')
    baseline.save_frequency_scores('../data/exp1/e1_data_long.csv')
